    ...
    /// <returns>A <see cref="{{TaskType}}"/> representing the new <see cref="{{ResultType}}{TResult, TError}"/> with the mapped value or the original error.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the mapper function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async {{TaskType}}<{{ResultType}}<TResult, TError>> MapAsync<TResult>(Func<T, {{TaskType}}<TResult>> mapper)
    {
        if (_isSuccess)
        {
    ...
    /// <summary>
    /// Asynchronously binds the successful value to a new <see cref="{{ResultType}}{TResult, TError}"/> using the provided binder function.
    /// If the result is a failure, it returns the error unchanged.
    /// </summary>
    ...
    /// <returns>A <see cref="{{TaskType}}"/> representing the new <see cref="{{ResultType}}{TResult, TError}"/> with the result of the binder or the original error.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the binder function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async {{TaskType}}<{{ResultType}}<TResult, TError>> BindAsync<TResult>(Func<T, {{TaskType}}<{{ResultType}}<TResult, TError>>> binder)
    {
        if (_isSuccess)
        {
    ...
    /// <returns>A <see cref="{{TaskType}}"/> representing the new <see cref="{{ResultType}}{T, TError}"/> with the original value if the predicate is satisfied, or a failure result if not.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the predicate or error factory function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async {{TaskType}}<{{ResultType}}<T, TError>> EnsureAsync(Func<T, {{TaskType}}<bool>> predicate, Func<{{TaskType}}<TError>> errorFactory)
    {
    ...
    /// <returns>A <see cref="{{TaskType}}"/> representing the current <see cref="{{ResultType}}{T, TError}"/> instance.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the action is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async {{TaskType}}<{{ResultType}}<T, TError>> OnSuccessAsync(Func<T, {{TaskType}}> action)
    {
        if (_isSuccess)
        {
    ...
    /// <exception cref="ArgumentNullException">Thrown when the action is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async {{TaskType}}<{{ResultType}}<T, TError>> OnFailureAsync(Func<TError, {{TaskType}}> action)
    {
        if (_isSuccess == false)
        {
            Debug.Assert(action is not null, "Action should not be null.");
    ...
    /// <returns>A <see cref="{{TaskType}}"/> representing the result of the executed function based on the result state.</returns>
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async {{TaskType}}<TResult> MatchAsync<TResult>(Func<T, {{TaskType}}<TResult>> onSuccess, Func<TError, {{TaskType}}<TResult>> onFailure)
    {
        if (_isSuccess)
        {
    ...
    /// <returns>A <see cref="{{TaskType}}"/> representing the completion of the executed action.</returns>
    /// <exception cref="ArgumentNullException">Thrown when either <paramref name="onSuccess"/> or <paramref name="onFailure"/> is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async {{TaskType}} MatchAsync(Func<T, {{TaskType}}> onSuccess, Func<TError, {{TaskType}}> onFailure)
    {
        if (_isSuccess)
        {
