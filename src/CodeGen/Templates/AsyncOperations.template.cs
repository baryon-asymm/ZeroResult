    ...
    where TError : IError
{
    ...
    /// <returns>A <see cref="ValueTask"/> representing the new <see cref="{{ResultType}}{TResult, TError}"/> with the mapped value or the original error.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the mapper function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async ValueTask<{{ResultType}}<TResult, TError>> MapAsync<TResult>(Func<T, ValueTask<TResult>> mapper)
    {
        if (_isSuccess)
        {
    ...
    /// <returns>A <see cref="ValueTask"/> representing the new <see cref="{{ResultType}}{TResult, TError}"/> with the result of the binder or the original error.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the binder function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async ValueTask<{{ResultType}}<TResult, TError>> BindAsync<TResult>(Func<T, ValueTask<{{ResultType}}<TResult, TError>>> binder)
    {
        if (_isSuccess)
        {
    ...
    /// <returns>A <see cref="ValueTask"/> representing the new <see cref="{{ResultType}}{T, TError}"/> with the original value if the predicate is satisfied, or a failure result if not.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the predicate or error factory function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async ValueTask<{{ResultType}}<T, TError>> EnsureAsync(Func<T, ValueTask<bool>> predicate, Func<ValueTask<TError>> errorFactory)
    {
    ...
    /// <returns>A <see cref="ValueTask"/> representing the current <see cref="{{ResultType}}{T, TError}"/> instance.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the action is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async ValueTask<{{ResultType}}<T, TError>> OnSuccessAsync(Func<T, ValueTask> action)
    {
        if (_isSuccess)
        {
    ...
    /// <exception cref="ArgumentNullException">Thrown when the action is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public async ValueTask<{{ResultType}}<T, TError>> OnFailureAsync(Func<TError, ValueTask> action)
    {
        if (_isSuccess == false)
        {
            Debug.Assert(action is not null, "Action should not be null.");
