    ...
    where TError : IError
{
    ...
    /// <returns>A new <see cref="{{ResultType}}{TResult, TError}"/> with the mapped value or the original error.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the mapper function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public {{ResultType}}<TResult, TError> Map<TResult>(Func<T, TResult> mapper)
    {
        if (_isSuccess)
        {
...
{{#if NET9_0_OR_GREATER}}
    /// <summary>
    /// Binds the successful value to a new <see cref="{{ResultType}}{TResult, TError}"/> using the provided binder function.
    /// If the result is a failure, it returns the error unchanged.
    /// </summary>
    ...
    /// <returns>A new <see cref="{{ResultType}}{TResult, TError}"/> with the result of the binder or the original error.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the binder function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public {{ResultType}}<TResult, TError> Bind<TResult>(Func<T, {{ResultType}}<TResult, TError>> binder)
    {
        if (_isSuccess)
        {
...
{{#endif}}
    ...
    /// <returns>A new <see cref="{{ResultType}}{T, TError}"/> with the original value if the predicate is satisfied, or a failure result if not.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the predicate or error factory function is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public {{ResultType}}<T, TError> Ensure(Func<T, bool> predicate, Func<TError> errorFactory)
    {
    ...
    /// <returns>The current <see cref="{{ResultType}}{T, TError}"/> instance.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the action is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public {{ResultType}}<T, TError> OnSuccess(Action<T> action)
    {
        if (_isSuccess)
        {
    ...
    /// <exception cref="ArgumentNullException">Thrown when the action is null.</exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public {{ResultType}}<T, TError> OnFailure(Action<TError> action)
    {
        if (_isSuccess == false)
        {
            Debug.Assert(action is not null, "Action should not be null.");
